...
When a public address is enabled, StartOS first **attempts to open the port automatically using UPnP** on the corresponding gateway. If the gateway supports UPnP (and it is enabled), no manual step is required — and when the address is later disabled or deleted, StartOS removes the port forward it created. UPnP is best-effort: if the gateway does not support it, the automatic test will fail and you create the rule manually as described below.
...
- **Routers**: Port forwarding is supported by all routers and easy to do. Many routers also support UPnP, in which case StartOS opens the port for you automatically. If UPnP is unavailable or disabled, refer to your router's manual to add the rule manually.

- **StartTunnel**: StartTunnel supports UPnP over the tunnel, so StartOS opens the required port automatically. To add or manage forwards manually, refer to the [StartTunnel Port Forwarding guide](/start-tunnel/port-forwarding.html).
