...
1. Enter a **Name** at the top of the dialog (e.g. "Admin", "Guest", "Children", "Smart Devices"). The Name field sits above three tabs — **LAN**, **Internet (WAN)**, and **DNS**.
...
1. On the **LAN** tab, configure local-network settings:
   ...
   - **Access control** — Controls which other profiles this profile can communicate with on the local network:
     ...
     - **Blacklist** — Select specific profiles to block; communicate with everything else.
...
1. On the **Internet (WAN)** tab, configure Internet access:
...
WAN Blackout is edited inline on the **Internet (WAN)** tab of the profile create/edit dialog (see [Creating a Profile](#creating-a-profile)).
